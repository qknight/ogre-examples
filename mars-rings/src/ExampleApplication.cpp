...
public:
    ...
    void createScene(void)
    {
        ...
        // now we are going to create the white rings ( a more complex example )
        // i want to illustrate the difference in quallity using the last parameter with steps: 4,16,32,64,128

        // this one looks really horrible since it only uses 4 sample points resulting in a rectangle not a ring... ;P
        CreateRing::create("ring0", mSceneMgr, Vector3(0,0,0), Vector3(0,1,0), 250, 4); 
        CreateRing::create("ring1", mSceneMgr, Vector3(0,0,0), Vector3(0,1,0), 500, 16);
        CreateRing::create("ring2", mSceneMgr, Vector3(0,0,0), Vector3(0,1,0), 750, 32);
        CreateRing::create("ring3", mSceneMgr, Vector3(0,0,0), Vector3(0,1,0), 1000, 64);
