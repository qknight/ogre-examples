...
public:
    ...
    bool buttonPressed( const OIS::JoyStickEvent &arg, int button ) {
        std::cout << "OIS::buttonPressed: " << button << std::endl;
        Light *light = mSceneMgr->getLight("Light1");
        switch (button) {
        case 14:
            light->setVisible(! light->isVisible());
    ...
    bool buttonReleased( const OIS::JoyStickEvent &arg, int button ) {
        std::cout << "OIS::buttonReleased: " << button << std::endl;
        return true;
    }

    bool axisMoved( const OIS::JoyStickEvent &arg, int axis ) {
      // if someone finds a fix to the issue i had - stuttering gamepad input - please report
      // it to me js@lastlog.de
      //         std::cout << "OIS::axisMoved at axis: " << axis << " " << arg.state.mAxes[axis].abs << std::endl;
      int a = arg.state.mAxes[axis].abs;
        if ( arg.state.mAxes[axis].abs != 0 ) {
            if (axis == 0)
                jDirection.x = a/50;
            if (axis == 1)
                jDirection.z = a/50;
...
protected:
    ...
    Vector3 jDirection;     // Value to move in the correct direction based on joystick input
};
